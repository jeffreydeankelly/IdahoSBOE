...
//             *    BigDecimal, String, String, String
             ...
             *    }
             *
             *  Below is an attempt at refactoring
			 *
    ...
    void concatSingleTokenPhrase(StringBuffer querySql, StringBuffer selectSql, String field, String token, int base) {
    	...
    	/*
    	 * 		Oracle syntax commented: RANK is omitted, which represents Oracle's "label" 
    	 * 		which is a number that identifies the score generated by the CONTAINS function.
    	 * 
         *   if (selectSql != null) {
         ...
         * of Oracle's "syn" (synonym) query operator.  RANK is also omitted, which represents Oracle's "label" which is 
         * a number that identifies the score generated by the CONTAINS function.
         */
         querySql.append(
           String.format(" CONTAINS(%1$s, '$%2$s')", field, token));
    ...
    void concatDoubleTokenPhrase(StringBuffer querySql, StringBuffer selectSql, String field, 
        ...
        */
        querySql.append(
                String.format(" CONTAINS(%1$s, '$%2$s NEAR $%3$s') ",
                        field, tokenA, tokenB));
    }
...
//                            field, tok1));
    ...
    List<SearchResult> searchGlossaryKeywords(EntityManager em,
        ...
        try {
			...
			*/
               List<Object[]> results = em.createNativeQuery(selectSql.toString()).getResultList();
               for (Object[] o: results) {
                   list.add(new SearchResult(new BigDecimal(String.valueOf(o[0])), 
                           o[1].toString(), o[2].toString(), o[3].toString()));
               }
